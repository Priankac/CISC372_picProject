...
int main(int argc,char** argv){
    ...
    int num_threads= 6;
    int row_thread= srcImage.height/num_threads;
    omp_set_thread(num_threads);

    #pragma omp parallel
    {
        int thread_in = omp_get_thread_num();
        int start = thread_id * row_thread;
        int end = (thread_id == num_threads - 1) ? srcImage.height : (thread_in + 1) * row_thread;
        rows(&srcImage, &destImage, type, start, end);
    }
